...
app = FastAPI(title="MuseTalk API", description="API for MuseTalk realtime inference")
...
class AvatarConfig(BaseModel):
    ...
    audio_clips: Dict[str, str] = Field(..., description="Dictionary of audio clips")
...
def initialize_models(config):
    ...
    global vae, unet, pe, whisper, audio_processor, fp, device, timesteps
...
async def run_inference(config: InferenceConfig, avatar_id: str, audio_id: str):
    ...
    try:
        ...
        # Get audio path
        audio_path = avatar_config.audio_clips[audio_id]
        ...
        output_path = avatar.inference(
            ...
            out_vid_name=audio_id if config.output_vid_name is None else config.output_vid_name,
            fps=config.fps,
